...
class ClassBaseService_S {
    ...
    #_Status;
    #_OriginalBusList = {};// объект-список инициализированных шин, полученный при создании
    #_Node;                // объект node
    ...
    #_ServicesState;       // объект состояний служб
    #_SourcesState;        // объект состояний источников
    #_BusList = {};        // объект-коллекция шин, используемых службой
    /**
     * @typedef {Object} ServiceOpts
     * @property {string} _name - имя службы
     * @property {[string]} _busNameList - список имен используемых шин
     * @property {object} _busList - глобальная коллекция инициализированных шин
     * @property {object} _advOpts - объект с дополнительными настройками службы
    ...
    constructor({ _name, _busNameList, _busList, _node }) {
        ...
        this.#_OriginalBusList = _busList;
    ...
    CreateBus(_busName) {
        if (this.Name === PROCESS_SERVICE_NAME && typeof _busName === 'string')
            this.#_OriginalBusList[_busName] = new ClassBus_S(_busName);
    }
    /**
     * @method
     * @private
     ...
     * В обработчике выполняется сохранение SourcesState и ServicesState, обновление активного списка шин.
     * @param {string} _topic 
     * @param {object} _msg 
     */
    ...
    UpdateBusList() {
        this.#_BusNameList.forEach(_busName => {
            this.#_BusList[_busName] ??= this.#_OriginalBusList[_busName];
        });
    }
    /**
     * @method
    ...
    CreateMsg(_msgOpts) {
        try {
            ...
            this.EmitEvents_logger_log({ level: 'E', msg: `Invalid msg format`, obj: _msgOpts });
            return null;
        }
    }
    /**
    ...
    async EmitMsg(_busName, _topic, _msg, _opts) {
        ...
        if (!msg) {
            this.EmitEvents_logger_log({ level: 'E', msg: `Unexpected msg format` });
            return false;
        }
